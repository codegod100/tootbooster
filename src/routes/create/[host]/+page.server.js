...
export async function load({ fetch, params, cookies }) {
	...
	let resp2 = await supabase
		.from('applications')
		.insert({ client_id: item.client_id, client_secret: item.client_secret, host })
	console.log(resp2.error, resp2.statusText)
	throw redirect(307, `https://${host}/oauth/authorize?client_id=${item.client_id}&scope=read+write+push&redirect_uri=${REDIRECT_URI}/${host}&response_type=code&host=${host}`)
}